...
class MatcherTest extends TestCase
{
	...
	public function testMatchType($value)
	{
		$result = pattern()
			(Type::array())     ->v((string)Type::array())
			(Type::bool())      ->v((string)Type::bool())
			(Type::float())     ->v((string)Type::float())
			(Type::int())       ->v((string)Type::int())
			(Type::null())      ->v((string)Type::null())
			(Type::object())    ->v((string)Type::object())
			(Type::string())    ->v((string)Type::string())
		->match($value);
		$this->assertSame((string)Type::of($value), $result);
	}
	...
	public function testDestructureList()
	{
		...
		$this->assertSame(
			null,
			$findUnfitNumber([1, 2, 3, 5, 8, 13, 21, 35, 56])
		);
		$this->assertSame(
			22,
			$findUnfitNumber([1, 2, 3, 5, 8, 13, 22, 35, 56])
		);
	}
